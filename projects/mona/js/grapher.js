/*global window, doc, document, Image, setInterval, clearInterval, Vector, getRandomArbitrary, setTimeout, doPhysical, graphWidth:true, graphHeight:true, primaryNodes:true, secondaryNodes:true, renderedNodesList:true, secondaryNodesList:true, nodePositions:true, typePositions, console, physicalInterval:true, MONA, updateLinePos*/
...
var ATTRACTION_FORCE = 1;
var REPULSE_FORCE = -10;
var TOUCH_DISTANCE = 80;
var Y_TOUCH_DISTANCE = 12;

//adjust these to stretch entire graph accross x/y axis
var X_REPULSE_FACTOR = 6;
var Y_REPULSE_FACTOR = 2;
...
function stepPhysical(x){
    ...
    for(n = 0; n < renderedNodesList.length; n++){
        ...
        if (primaryNodes.hasOwnProperty(node.location) && renderedNodesList.length == Object.keys(primaryNodes).length && Object.keys(primaryNodes).length < 50){
		...
		for(p = 0; p <renderedNodesList.length; p++){		
			if(n != p){
                ...
                else if(yDist < Y_TOUCH_DISTANCE){
					var oSquare = other.visual.getBoundingClientRect();
					var nSquare = node.visual.getBoundingClientRect();
					if ((nSquare.right > oSquare.left && nSquare.right < oSquare.right) || (oSquare.right > nSquare.left && oSquare.right < nSquare.right)){
						pY = (other.y - node.y) / pDist;
						pSpeed =  ((TOUCH_DISTANCE - yDist) / TOUCH_DISTANCE) * REPULSE_FORCE; 
						pY *= pSpeed * Y_REPULSE_FACTOR;
...
function moveNode(n){
    if (n !== undefined){
        ...
        var oFactor = 100 / graphWidth;
        oFactor = (oFactor*z)+20;
        if (oFactor > 99) oFactor = 99;
        
        n.visual.style.opacity = '.' + Math.round(oFactor);
...
function updateAllLines(){
	for (var nodeKey in primaryNodes){
		...
		var line = document.getElementById(nodeKey+"Line");
        updateLinePos(line, nodePositions[nodeKey].left + leftOffset + 5, nodePositions[nodeKey].top+nodePositions[nodeKey].height/2 + topOffset);
        ...
        for (var i in primaryNodes[nodeKey].children){
            ...
            if (div2 !== null){
                ...
                if (line2 !== null){
                    var x1 = nodePositions[childKey].left + leftOffset;
                    var x2 = nodePositions[nodeKey].left + 2 + leftOffset;
                    var y1 = nodePositions[childKey].top + nodePositions[childKey].height/2 + topOffset;
                    var y2 = nodePositions[nodeKey].top + nodePositions[nodeKey].height/2 + topOffset;
                    updateLinePos(line2, x1, y1, x2, y2);
                }
            }
        }
	}
...
function updateNodeLines(node){
    ...
    if (primaryNodes.hasOwnProperty(node.location)){
        ...
        if (line !== null){
            updateLinePos(line, nodePositions[node.location].left + leftOffset + 5, nodePositions[node.location].top+nodePositions[node.location].height/2 + topOffset);
        }
    }
    ...
    for (var i in node.children){
        ...
        if (div !== null){
            ...
            if (line !== null){
                x1 = nodePositions[childKey].left + leftOffset;
                x2 = nodePositions[node.location].left + 2 + leftOffset;
                y1 = nodePositions[childKey].top + nodePositions[childKey].height/2 + topOffset;
                y2 = nodePositions[node.location].top + nodePositions[node.location].height/2 + topOffset;
    ...
    for (var p in node.parents){
        ...
        if (div !== null){
            ...
            if (line !== null){
                x1 = nodePositions[parentKey].left + leftOffset;
                x2 = nodePositions[node.location].left + 2 + leftOffset;
                y1 = nodePositions[parentKey].top + nodePositions[parentKey].height/2 + topOffset;
                y2 = nodePositions[node.location].top + nodePositions[node.location].height/2 + topOffset;
                updateLinePos(line, x1, y1, x2, y2);
            }
        }
