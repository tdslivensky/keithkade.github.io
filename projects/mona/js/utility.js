/*global MDC_rawMMD, MDC_rawMetadata, HISTORY_ELEM, console, document*/
...
function updateLinePos(line, x1, y1, x2, y2){
    if (x1) line.setAttribute('x1', x1);
    if (x2) line.setAttribute('x2', x2);
    if (y1) line.setAttribute('y1', y1);
    if (y2) line.setAttribute('y2', y2);
    
    var hovLine = document.getElementById(line.id + "Hover");
    if (hovLine){
        if (x1) hovLine.setAttribute('x1', x1);
        if (x2) hovLine.setAttribute('x2', x2);
        if (y1) hovLine.setAttribute('y1', y1);
        if (y2) hovLine.setAttribute('y2', y2);
    }
}
